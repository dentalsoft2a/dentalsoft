...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const threeshapeApiUrl = Deno.env.get("THREESHAPE_API_URL") || "https://api.3shapecommunicate.com";
    ...
    if (credError || !credentials) {
      return new Response(
        JSON.stringify({ error: "3Shape Communicate not connected" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    if (now >= tokenExpiry) {
      const refreshResponse = await fetch(`${Deno.env.get("THREESHAPE_AUTH_URL")}/oauth/token`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          grant_type: "refresh_token",
          refresh_token: credentials.refresh_token,
          client_id: Deno.env.get("THREESHAPE_CLIENT_ID"),
          client_secret: Deno.env.get("THREESHAPE_CLIENT_SECRET"),
        }),
      });

      if (!refreshResponse.ok) {
        throw new Error("Failed to refresh 3Shape Communicate token");
      }
    ...
    const filesResponse = await fetch(`${threeshapeApiUrl}/v1/files/new?${params.toString()}`, {
      headers: {
    ...
    if (!filesResponse.ok) {
      throw new Error("Failed to fetch files from 3Shape Communicate");
    }

    const filesData = await filesResponse.json();
    const files: ThreeShapeFile[] = filesData.files || [];
    ...
    for (const file of files) {
      try {
        ...
        const { data: mapping } = await supabase
          ...
          .eq("user_id", userId)
          .eq("threeshape_dentist_id", file.dentistId)
          .maybeSingle();
        ...
        if (!localDentistId) {
          ...
          await supabase
            ...
            .upsert({
              user_id: userId,
              threeshape_dentist_id: file.dentistId,
              threeshape_dentist_name: file.dentistName,
              threeshape_dentist_email: file.dentistEmail,
        ...
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            photo_url: publicUrl,
            notes: `Synchronisé depuis 3Shape Communicate - ${file.fileName}`,
            status: "pending",
            source: "3shape",
            threeshape_id: file.id,
            threeshape_metadata: file.metadata || {},
          });
