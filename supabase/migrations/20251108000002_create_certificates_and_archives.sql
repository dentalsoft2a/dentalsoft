/*
  ...
  1. Nouvelles tables
    - `digital_certificates`
      - Certificats numériques par laboratoire
      - Clés RSA pour signature électronique
      - Clé privée chiffrée

    - `archived_documents`
      - Documents archivés (factures, avoirs)
...
CREATE INDEX IF NOT EXISTS idx_digital_certificates_valid ON digital_certificates(valid_until) WHERE valid_until > now();

-- RLS
ALTER TABLE digital_certificates ENABLE ROW LEVEL SECURITY;

-- Politique: Les utilisateurs peuvent voir leur propre certificat
...
-- Politique: Les utilisateurs peuvent créer leur certificat (une seule fois)
CREATE POLICY "Users can create own certificate"
  ON digital_certificates
  FOR INSERT
  TO authenticated
  WITH CHECK (
    laboratory_id IN (SELECT id FROM profiles WHERE id = auth.uid())
    AND NOT EXISTS (
      SELECT 1 FROM digital_certificates
      WHERE laboratory_id = auth.uid()
    )
  );
...
-- Politique: Les utilisateurs peuvent voir leurs propres archives
...
-- Politique: System peut créer des archives (via edge functions)
...
CREATE OR REPLACE FUNCTION archive_document(
  ...
  p_file_size BIGINT,
  p_file_hash TEXT,
  p_facturx_xml TEXT DEFAULT NULL,
...
BEGIN
  ...
  INSERT INTO archived_documents (
    ...
    v_document.laboratory_id,
    p_document_type,
    p_document_id,
    v_document.document_number,
    v_document.fiscal_year,
    v_document.fiscal_month,
    p_storage_path,
    p_file_size,
    p_file_hash,
    p_facturx_xml,
    p_digital_signature,
    calculate_retention_date(v_document.date)
  )
