/*
  # Fix Invoice Number Generation with RLS
  
  This migration creates a secure function to generate unique invoice numbers
  that bypasses RLS restrictions to ensure global uniqueness.
  
  1. New Function
    - `generate_next_invoice_number()` - Returns the next available invoice number
      - Runs with SECURITY DEFINER to bypass RLS
      - Ensures global uniqueness across all users
      - Returns format: INV-YYYY-NNNN
...
DROP FUNCTION IF EXISTS generate_next_invoice_number();

-- Create function to generate next invoice number (bypasses RLS)
