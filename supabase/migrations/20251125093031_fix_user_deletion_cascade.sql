/*
  # Fonction de suppression complète d'utilisateur

  1. Création
    - Fonction PostgreSQL qui supprime tout en une seule transaction
    - Contourne les politiques RLS avec SECURITY DEFINER
    - Supprime dans le bon ordre pour éviter les violations de contraintes

  2. Sécurité
    - Accessible uniquement aux super admins
    - Utilise SECURITY DEFINER pour exécuter avec les privilèges du propriétaire
*/

-- Créer une fonction qui supprime un utilisateur et toutes ses données
CREATE OR REPLACE FUNCTION delete_user_and_all_data(target_user_id UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Tables liées à user_profiles
  DELETE FROM admin_impersonation_sessions WHERE admin_user_id = target_user_id OR target_user_id = target_user_id;
  ...
  DELETE FROM photo_submissions WHERE laboratory_id = target_user_id;
  ...
  DELETE FROM delivery_notes WHERE user_id = target_user_id;
  DELETE FROM dentist_favorite_laboratories WHERE laboratory_profile_id = target_user_id;
  DELETE FROM dentists WHERE user_id = target_user_id;
  ...
  DELETE FROM stl_files WHERE laboratory_id = target_user_id;
...
GRANT EXECUTE ON FUNCTION delete_user_and_all_data(UUID) TO service_role;
