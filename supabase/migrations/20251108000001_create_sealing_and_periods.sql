/*
  ...
  2. Sécurité
    - RLS activé
    - Seul le propriétaire peut voir ses périodes
...
-- Politique: Les utilisateurs peuvent voir leurs propres scellements
...
-- Politique: Les utilisateurs peuvent voir leurs propres périodes
...
-- Politique: Les utilisateurs peuvent créer leurs périodes
...
-- Politique: Les utilisateurs peuvent mettre à jour leurs périodes ouvertes
...
CREATE OR REPLACE FUNCTION seal_fiscal_period(
  p_period_id UUID,
...
BEGIN
  ...
  -- Marquer les enregistrements d'audit comme scellés
  UPDATE audit_log
  SET is_sealed = true, seal_id = v_seal_id
  WHERE laboratory_id = v_period.laboratory_id
    AND created_at BETWEEN v_period.period_start AND v_period.period_end
    AND entity_type IN ('invoices', 'credit_notes');
  ...
  -- Verrouiller toutes les factures de la période
  UPDATE invoices
  SET
    is_locked = true,
    locked_at = now(),
  ...
  -- Verrouiller tous les avoirs de la période
  UPDATE credit_notes
  SET
    is_locked = true,
    locked_at = now(),
