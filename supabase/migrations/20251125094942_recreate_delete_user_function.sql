/*
  # Recreate delete user function with fixed parameter names

  1. Changes
    - Supprime l'ancienne fonction
    - Recrée avec des noms de paramètres non ambigus (préfixe p_)
*/

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS public.delete_user_and_all_data(uuid, uuid);

-- Recréer avec les nouveaux noms de paramètres
...
BEGIN
  ...
  DELETE FROM audit_log WHERE laboratory_id = p_target_user_id;
  ...
  DELETE FROM user_extensions WHERE profile_id = p_target_user_id;
