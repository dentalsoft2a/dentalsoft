...
interface ThreeShapeMapping {
  id: string;
  threeshape_dentist_id: string;
  threeshape_dentist_name: string;
  threeshape_dentist_email: string;
...
export default function ThreeShapeDentistMapping() {
  const { user } = useAuth();
  const [loading, setLoading] = useState(true);
  const [mappings, setMappings] = useState<ThreeShapeMapping[]>([]);
  ...
  const loadMappings = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('threeshape_dentist_mapping')
        .select('*')
        .eq('user_id', user.id)
        .order('threeshape_dentist_name');
  ...
  const filteredMappings = mappings.filter(
    (mapping) =>
      mapping.threeshape_dentist_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      mapping.threeshape_dentist_email.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl md:text-2xl font-bold text-slate-900">Mapping Dentistes 3Shape Communicate</h2>
        <p className="text-slate-600 mt-1 text-sm md:text-base">Associez les dentistes 3Shape à vos comptes locaux</p>
      </div>
        ...
        <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-800">
              Des dentistes 3Shape Communicate n'ont pas de compte local associé. Configurez-les pour une meilleure organisation.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Rechercher un dentiste 3Shape Communicate..."
      ...
      {filteredMappings.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-2xl border border-slate-200">
          ...
          <h3 className="text-lg font-semibold text-slate-900 mb-2">Aucun dentiste 3Shape Communicate</h3>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="bg-gradient-to-r from-green-600 to-teal-600 px-6 py-4">
            <h2 className="text-xl font-bold text-white">Dentistes 3Shape Communicate</h2>
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <h3 className="font-semibold text-slate-900">{mapping.threeshape_dentist_name}</h3>
                          <p className="text-sm text-slate-600">{mapping.threeshape_dentist_email}</p>
                        </div>
                      </div>
