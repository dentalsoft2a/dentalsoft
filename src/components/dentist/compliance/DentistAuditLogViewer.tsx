...
export default function DentistAuditLogViewer() {
  const { user } = useAuth();
  ...
  }, [user, filterType, filterOperation]);
  ...
  const verifyIntegrity = async () => {
    ...
    try {
      const { data, error } = await supabase
        .rpc('verify_dentist_audit_chain', {
  ...
  const exportAuditLog = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `audit-log-dentiste-${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
  ...
  const getEntityTypeLabel = (type: string) => {
    switch (type) {
      case 'dental_invoices':
        return 'Facture';
      case 'dental_payments':
        return 'Paiement';
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-600 mb-4">
              Toutes les opérations sur vos factures et paiements sont enregistrées de manière chronologique et sécurisée.
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-4">
        <div className="flex flex-wrap gap-4">
          ...
          <select
            ...
            className="px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm"
          >
            <option value="all">Tous les types</option>
            <option value="dental_invoices">Factures</option>
            <option value="dental_payments">Paiements</option>
            <option value="dental_patients">Patients</option>
          </select>
