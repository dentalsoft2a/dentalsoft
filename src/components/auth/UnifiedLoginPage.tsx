...
export default function UnifiedLoginPage({ onNavigate }: UnifiedLoginPageProps) {
  ...
  const getTitle = () => {
    if (isRegisterMode) {
      return userType === 'dentist' ? 'Inscription Dentiste' : 'Créer un compte';
    }
    return 'Connexion';
  };
  ...
  const getSubtitle = () => {
    if (isRegisterMode) {
      return userType === 'dentist' ? 'Créez votre compte dentiste' : 'Créez votre compte laboratoire';
    }
    return 'Accédez à votre espace';
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-2xl p-8 border border-slate-200">
          ...
          {!isRegisterMode || userType === 'laboratory' ? (
