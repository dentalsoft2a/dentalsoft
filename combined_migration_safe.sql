...
DROP POLICY IF EXISTS "Authenticated users can upload dentist photos" ON storage;
CREATE POLICY "Authenticated users can upload dentist photos"
  ON storage.objects FOR INSERT
  TO authenticated
  WITH CHECK (bucket_id = 'dentist-photos');

DROP POLICY IF EXISTS "Anyone can view dentist photos" ON storage;
CREATE POLICY "Anyone can view dentist photos"
  ON storage.objects FOR SELECT
  TO authenticated
  USING (bucket_id = 'dentist-photos');

DROP POLICY IF EXISTS "Users can update their own uploads" ON storage;
CREATE POLICY "Users can update their own uploads"
  ON storage.objects FOR UPDATE
  TO authenticated
  USING (bucket_id = 'dentist-photos' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "Users can delete their own uploads" ON storage;
